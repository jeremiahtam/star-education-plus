    ...
    <>
      <Formik
        ...
        initialValues={{
          ...
          membershipPlanContent: selectedMembershipPlan
            ? selectedMembershipPlan?.membershipPlanContent
            : "", //?.replace(/\n/g, "<br>"),
          duration: selectedMembershipPlan
            ? selectedMembershipPlan?.duration
            : "",
          amount: selectedMembershipPlan ? selectedMembershipPlan?.amount : "",
          discountAmount: selectedMembershipPlan
            ? selectedMembershipPlan?.discountAmount
            : "",
          discountFrequency: selectedMembershipPlan
            ? selectedMembershipPlan?.discountFrequency
            : "",
          status: selectedMembershipPlan ? selectedMembershipPlan?.status : "",
          freebies: selectedMembershipPlan?.freebies
            ? selectedMembershipPlan?.freebies
            : [],
        }}
        validationSchema={Yup.object({
          ...
          discountAmount: Yup.number()
            .typeError("Enter discount price")
            .required("Amount cannot be empty!")
            // .positive('Enter a positive number')
            .test(
              "maxDigitsAfterDecimal",
          ...
          freebies: Yup.array()
            .of(
              Yup.object().shape({
                ...
                packagesAndServicesId: Yup.number()
                  .typeError("Enter a number")
                  .required("Required!")
                  .positive("Positive number"),
                frequency: Yup.string().required("Required!"),
                freeAttempts: Yup.number()
                  .typeError("Enter a number")
                  .required("Required!")
                  .positive("Positive number"),
              })
          ...
          >
            ...
            <Modal.Body>
              ...
              <Form.Group className="mb-3">
                ...
                <Field
                  className="form-control custom-text-input"
                  placeholder="Full details of membership plan"
                  as="textarea"
                  rows={4}
                  style={{ whiteSpace: "pre-wrap" }}
                  name="membershipPlanContent"
                  id="membershipPlanContent"
                  disabled={isSubmitting}
                />
                <div className="form-error">
                      ...
                      <Row className="mb-3 align-items-center" key={index}>
                        ...
                        <Col xs={6} md={3}>
                          <Form.Group className="mb-3">
                            ...
                            <Form.Select
                              className="custom-text-input"
                              name={`freebies.${index}.packagesAndServicesId`}
                              onChange={(selectedOption: any) => {
                                setFieldValue(
                                  `freebies.${index}.packagesAndServicesId`,
                                  selectedOption.target.value
                                );
                              }}
                        ...
                        <Col xs={6} md={3}>
                          <Form.Group className="mb-3">
                            ...
                            <Form.Select
                              onChange={(selectedOption: any) =>
                                setFieldValue(
                                  `freebies.${index}.frequency`,
                                  selectedOption.target.value
                                )
                              }
                              className="custom-text-input"
                              name={`freebies.${index}.frequency`}
                              value={freebie.frequency}
                            >
                              <option value="">-- select --</option>
                              <option value="per-cycle">
                                Every subscription
                              </option>
                              <option value="per-lifetime">
                                Once in a lifetime
                              </option>
                            </Form.Select>
                            <div className="form-error">
                        ...
                        <Col xs={5} md={2}>
                          <Form.Group className="mb-3">
                            ...
                            <Field
                              ...
                              type="text"
                              placeholder="Eg. 2"
                              name={`freebies.${index}.freeAttempts`}
                              disabled={isSubmitting}
                            />
                            <div className="form-error">
                              <ErrorMessage
